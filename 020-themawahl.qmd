...
%%| label: fig-litsearch
flowchart LR
Suchen-->Screenen-->Sammeln-->Lesen-->Zusammenfassen-->Suchen
```
...
1. *Suchen* (Literaturrecherce) ist das Ausfindigmachen geeigneter Literatur. Dazu sucht man meist in einschlägigen wissenschaftlichen Literatur-Datenbanken wie z. B. Google Scholar.
...
5. Zentrale Inhalte *fasst* man aus dem Werk *zusammen* und hält sie an geeigneter Stelle fest, z. B. als Tag oder Notiz in Zotero oder im [Zettelkasten](https://zettelkasten.de/overview/).
...
Der Ablauf einer Literaturrecherche ist in @fig-litsearch-ablauf dargestellt.



```{mermaid}
%%| fig-cap: Der Ablauf der Literaturrecherche
%%| label: fig-litsearch-ablauf
flowchart LR
  Suchen --> Screenen --> Sammeln --> Z[Zusammenfassen] --> Verbinden 

```


1. *Suchen*: Sie finden Literaturquellen, die für Ihre Forschungsfrage relevant sein könnnten.
2. *Screenen*: Sie verschaffen sich einen Überblick über Passung von Forschungsfrage sowie methodischer Angemessenheit der identifizierten Literaturquellen.
3. *Sammeln*: Sie halten die Quellenangaben geeigneter Werke in einer Liste fest (in Ihrem Literaturverwaltungsprogramm).
3. *Zusammmenfassen*: Sie schreiben einen Überblick zum Inhalt der relevanten Literaturquellen. Sie schreiben also das Wesentliche der Quelle zusammen.
4. *Verbinden*: Sie setzen die zentralen Inhalte der Literaturquellen so miteinander in Bezug, dass Sie eine "Landkarte" von Wissen (inkl. der Quellenangaben) bekommen.
...
Geben Sie eine Liste von 5 hoch relevanten Papers an (am besten nach APA zitiert). 
Sie müssen *keine* Volltexte bereitstellen. 
Sie müssen die Quellen auch nicht lesen.
...
Nur wenn Sie über schriftliche Notizen verfügen, können Sie auch nach längerer Zeit auf das Wissen zugreifen.
...
Aber Notizen alleine nutzen nicht viel.
Viele Notizen für sich genommen nutzen nicht viel.
Viele Notizen, ohne Verbindung, sind nur ein Haufen.
...
![Die Oberfläche von Obsidian](img/obsidian1.png)
...
:::callout-caution
Beachten Sie Datenschutz und Urheberrechte, wenn Sie Texte wie Fachartikel an Dritte weitergeben. 
...
Die Rechtsprechung ist noch nicht ausgegoren, da es sich um neue und dynamische Entwicklungen handelt.
Vermutlich ist aber die Gefahr gering, belangt zu werden. $\square$
:::
