# Auswerten: Grundlegende Datenanalyse
...
Das diente nur zu Übungszwekcen.
In anderen Situationen kann es aber nötig sein, Items umzukodieren. 
Vergessen Sie dann nicht, das Ergebnis als (neuen) Datensatz zu speichern.
...
Der Wert von `extra_sum` berechnet sich jeweils als Summe der drei Itemwerte. Mit dem Mittelwert verhält es sich analog (s. @tbl-extra-score-mean).
...
```{r}
#| label: Extraversion-Score berechen
#| echo: false
#| tbl-cap: tbl-extra-score-mean
...
```
 ...
 zum Beispiel im Paket `sjmisc`: `row_sums()` (Summenscore pro Person) und `row_means()` (Mittelwert pro Person). 
...
Das kann man mit `normalize` aus dem Paket easystats erledigen.

```{r}
extra %>% 
  mutate(extra_mean_01 = normalize(extra_mean)) %>% 
  select(extra_mean, extra_mean_01) %>% 
