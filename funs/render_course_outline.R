render_course_outline <- function(yml_file, header_level = 2){
  ...
  modul_zsfg <- yaml::read_yaml(yml_file)
  ...
  for (i in seq_along(chapters)){
    ...
    # oberabschnitt:
    if (!is.null(modul_zsfg[[i]][["Titel"]])) titel <- modul_zsfg[[i]][["Titel"]]
    if (!is.null(modul_zsfg[[i]][["title"]])) titel <- modul_zsfg[[i]][["title"]]
    if (!is.null(modul_zsfg[[i]][["titel"]])) titel <- modul_zsfg[[i]][["titel"]]
    ...
    render_section(name = "Uebung")
    render_section(name = "Plenum")
