...
Teilen Sie sich dabei in Gruppen auf; jede Gruppe nutzt eine andere KI, s. @tbl-ki-summ-pdf. $\square$
:::
...
: KD-Dienste zur Textzusammenfassung (Auswahl){#tbl-ki-summ-pdf .striped .hover}
...
Daher kann es Ihre Ergebnisse verbessern, wenn Sie einer KI nicht ein PDF zum Zusammenfassen geben (besonders, wenn es sehr lang ist), sondern eine Textdatei, die eben nur den zusammenfassenden Text enthält, s. @lst-extract-pdf.
...
#| eval: false
library(pdftools)  # ggf. vorab installieren

# Text aus PDF extrhaieren:
