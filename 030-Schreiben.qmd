...
- Sie wissen, wie man einen Theorieteil aufbaut.
...
das macht Ihnen die Arbeit leichter, sofern Sie diesen Aufbau kennen.
...
Allerdings sind auch andere Untergliederungen Ihres Theorieteils möglich.
...
\beta_1 &\sim N(0, 2.5)  \qquad \text{Prior Regressionsgewicht lern} \\
\beta_2 &\sim N(0, 2.5)  \qquad \text{Prior Regressionsgewicht mot} \\
...
Praktischerweise müssten sie nicht mal ihre Variablen z-transformieren (aber Sie können ohne Schaden!),
...
Dabei meint $\beta$ ein Regressionsgewicht,
...
schon aufgrund der natürlichen Ambiguität der Sprache, mit Hilfe mathematischer Notation zu präzisieren:

- "Der Zusammenhang $\beta$ ist definiert als das Regressionsgewicht der Variable `lern` im Modell `m1`.
...
- "Der statistische Effekt ist definiert als das Regressionsgewicht der experimentellen Bedingung (binäre Variable `group`) im Modell `m1`.
...
sondern lediglich so objektiv wie möglich Tatsachen (Fakten) berichten.
...
### "Wander-Übung": Diskutieren Sie mit Ihren Mitstudis den Aufbau einer Forschungsarbeit!
