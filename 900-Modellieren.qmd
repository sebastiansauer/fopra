...
Die Punkt- und Intervallschätzer (95%-ETI) sind in @fig-m1-params dargestellt.
...
Der Gruppenunterschied wird auf `r round(coef(m_bringtnixtin), 2)` geschätzt;
das ist der Punktschätzer der UV `g`.
Die Grenzen eines 95%-CI für die UV liegen bei `r m1_lo_ci` bzw. `r m1_high_ci`.
...
Wir modellieren (in `m_bringtnixtin2`) jetzt die Veränderung `d = y2 - y1` als AV; UV ist wieder `g`, s. @tbl-m-bringtnixtin2-params.
...
Gängige Forschungsfragen für Beobachtungsstudien sind [hier](https://start-bayes.netlify.app/https://start-bayes.netlify.app/1000-metrische-av) erläutert.
...
Nach jeder Bedingung wird die Behaltensleistung gemessen (anhand von 10 Wissensfragen, die jeweils als "richtig" oder "falsch" bewertet wurden), mit `y1` nach `D.1` und `y2` für `D.2`.$\square$
:::
...
%%| fig-cap: "Ablauf der Studie zur Behaltensleistung y2 in Abhängigkeit der Diagrammart D (Within-Variable)"
flowchart LR
  D.1 --> y1 --> D.2 --> y2
```
...
#| fig-cap: "DAG für die Studie zur Behaltensleistung y2 in Abhängigkeit der Diagrammart D"
#| echo: false
...
%%| fig-cap: "Ablaufdiagramm für die Studie mit der Modellgleichung mit einem Within- und einem Between-Faktor"
flowchart LR
  subgraph g2
    direction LR
    D.1 --> y1 --> D.2 --> y2
...
#| fig-cap: "DAG für die Studie mit der Modellgleichung mit einem Within- und einem Between-Faktor. Es wird kein Effekt für g erwartet, wohl aber ein Effekt für D"
#| echo: false
