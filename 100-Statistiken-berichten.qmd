...
Dabei sei hinzugefügt, dass Statistik, so nüchtern sie den geneigten Studentis auch erscheinen mag,
durchaus Partikularitäten aufweist:
Nicht jeder Autor oder Dozentin oder jede Richtlinie gibt die gleichen Anweisungen oder Empfehlungen!
Die folgenden Hinweise entsprechen der (aktuellen, sich durchaus im Lauf der Zeit ändernden) Sicht dieses Dozenten.


- *Prägnanz*: Das *erste* Prinzip des Berichtlegens lautet "so viel wie nötig, aber so wenig wie möglich".
Man will die Lesis nicht überfrachten, aber alle nötigen Informationen übermitteln.

- *Ordnung*: Das *zweite* Prinzip lautet, dass man die Informationen am rechten Ort vermittelt.
...
- *Priorisierung*: Das *dritte* Prinzip lautet, dass man priorisiert. 
...
- *Konsistenz*: Das *vierte* Prinzip lautet, konsistent zu sein. 
...
- *Transparenz*: Das *fünfte* Prinzip, könnte man sagen,  ist so selbstverständlich,
dass es keiner Erwähnung bedarfe, aber die Realität lehrt uns leider mitunter das Gegenteil.
Es lautet Transparenz, Lauterkeit oder Rechtschaffenheit.
...
Auch die APA (American Psychological Assocation) hat eine [Checkliste herausgegeben](https://sebastiansauer.github.io/data_se/2022/01/27/warum-bayes/),
...
Vielmehr soll - angepasst an den Kenntnisstand von Bachelor-Studentis - ein
angemessener Überblick ausgewählt werden.
Ambitionierte Studentis sind aufgefordert, breite und tiefer als in der folgenden Ausführung erläutert,
zu berichten.
...
Falls Sie die Daten aufbereiten mussten in einem Maße, dass übliche, notwendige Operationen übersteigt, 
sollten Sie kurz berichten, was Sie getan haben. 
Wenn Sie lediglich die Daten mit Operationen wie Scores berechnet oder Variablen zentriert aufbereitet haben, 
müssen Sie nichts weiter berichten. 
...
Die Stichprobengröße sollte klar berichtet sein; 
...
Gängige statistische Symbole sollen nicht definiert werden, 
z.B. *M*, *SD*, *F*, *t*, *df*, *N*, *n*, *OR*.  
Andere statistischen Abkürzungen, die weniger gebräuchlich sind,
sollten definiert bzw. auf die Definition verwiesen werden, z.B. *pd*, *ROPE*.


*Formulierungsvorschlag*

>   Der mittlere Achtsamkeitswert lag in der Stichprobe bei *M* = 12.23 (*SD* = 1.23).
...
mtcars %>% 
  ...
  rename(`MW Spritverbrauch` = mpg_avg,
         ...
         `Korrelation Spritverbrauch mit PS-Zahl` = cor_mpg_hp) |> 
          tt() |> 
          format_tt(digits = 2, num_fmt = "decimal", num_mark_dec = ",")
```



Alternativ zu selbsterstellten Tabellen kann "Statistik-Fast-Food" konsumieren 
und lässt sich einen Haufen Zahlen auf einmal ausgeben.
...
Tabellen mit `flextable` oder `tinytable` kann man sich eine schicke Tabelle (im HTML- oder PDF-Format) ausgeben lassen,
...
meine_tab <- 
  describe_distribution(mtcars) %>% 
  tt() %>%  # erzeugt schicke Tabelle mit tinytable
  format_tt(digits = 2, num_fmt = "decimal", num_mark_dec = ",") |> 
  theme_striped()  # "gestreifte" Tabellen
...
Verzichten sollte man aber vermutlcih - in diesem Fall - auf die Spalte `n_Missing`,
da die Spalte keine Information birgt.



[Hier](https://sebastiansauer.github.io/data_se/2022/05/02/kontigenztabellen-in-r/#exportieren) findet sich noch mehr zum Thema Exportieren von Tabellen aus R nach Word.
...
summary(meine_cor_tab) %>% 
  tt() %>%  # machen wir gleich eine schicke HTML-Tabelle
  format_tt(digits = 2, num_fmt = "decimal", num_mark_dec = ",")
```
...
### Inferenzstatistik und Modellierung

Es empfiehlt sich, die Modellgleichung inkl. Prior-Spezifikation aufzuführen.
...
m1_frequentistisch <- lm(mpg ~ am, data = mtcars)
```

Hier sind die Ergebnisse; schon ordentlich poliert für einen APA-Bericht, 
s. @tbl-m1-params.
...
>    Der statistische Effekt der Getriebeart auf den Kraftstoffverbrauch wurde vom Modell auf 7.21 geschätzt, 95%-KI [3.72, 10.70]. Da die Null nicht im Intervall liegt, kann die Nullhypothese verworfen werden.


Die APA würde noch den *t*- und den *p*-Wert erwarten:

>    Der statistische Effekt der Getriebeart auf den Kraftstoffverbrauch wurde vom Modell auf 7.21 geschätzt, 95%-KI [3.72, 10.70]. Da die Null nicht im Intervall liegt, kann die Nullhypothese verworfen werden, t(30) = 4.11, p < .001.
...
Den Punktschätzer (`Median`) zum Unterschied für die Gruppen (Automatik vs. Schaltgetriebe) hat uns die Funktion `parameters()` auch geliefert. Der Unterschied zwischen den beiden Gruppen liegt laut Modell bei ca. 7.2 Meilen pro Gallone Kraftstoff.


Zusätzlich sollte man noch den Anteil der erklärten Varianz angeben:

```{r}
r2(m1_frequentistisch)
r2(m1)
```

>   Das Modell erklärte 36 Prozent der Varianz (95% CI [0.100, 0.542]).


Nur bei Bayes-Modellen kann man zusätzlich ein Konfidenzintervall zum Punktschätzer von $R^2$ angeben.


Mit `apaTables` kann man sich eine Tabelle für ein frequentistisches Modell geben lassen.

```{r}
library(apaTables)

apa.reg.table(m1_frequentistisch, filename = NA)
```

Interessant ist die *quadrierte semipartielle Korrelation*, $sr^2$,
die den Anteil der erklärten Varianz pro UV angibt.
...
Möchte man verschiedenen Regressionsgewichte vergleichen,
...
Mit "Verwerfen" ist das Verwerfen der "Praktisch-Null-Hypothese" gemeint.
...
Um $R^2$ zu bekommen, bietet sich die Funktion `r2()` an (sowohl für Bayes- als auch für frequentistische Modelle).
...
Der genaue Wert bei `seed` ist nicht entscheidend;
...
(leider können sich die Werte zwischen verschiedenen Computern unterscheiden).
