...
In dieser Tabelle wurden anstelle von "sprechenden" Variablennamen Abkürzungen verwendet.
Das ist, wenn Sie einen Bericht schreiben, nicht sinnvoll.
In einem Bericht sollten besser verständliche Variablennamen stehen (oder zur Not die Abkürzungen erklärt sein).
...
Möchten man eine *quadratische* Korrelationstabelle (was der üblicheren Berichtsform entspricht) kann man das so bekommen:
...
parameters(m1, prob = .95, ci_method = "hdi")  # oder: hdi(m1)
```
