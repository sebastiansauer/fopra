...
library(mediation)  # Mediation, ggf. weitere Pakete, s.u.
```
...
ggbetweenstats(
  ...
  labs(y = "fluide Intelligenz",
       x = "Gruppe") +
    scale_x_discrete(labels = c("0" = "Kontrollgruppe", 
...
ggboxplot(
  ...
  labs(caption = "Die Punkte stellen den Mittelwert pro Gruppe dar.") +
    scale_x_discrete(labels = c("0" = "Kontrollgruppe", 
                              "1" = "Experimentalgruppe")) 
```
...
```{r}
#| label: fig-bringtnixtin-desk3
#| echo: false
#| fig-cap: "Fluide Intelligenz (y2) nach der Bringtnixtin-Intervention. g=0: Kontrollgruppe (Placebo), g=1: Experimentalgruppe (hohe Dosis).Die vertikale Linie zeigt die Stärke des Effekts (entspricht dem Unterschied in den Mittelwerten der Gruppen)"


ggplot(data = d_bringtnixtin,
       aes(x = factor(g), y = y2)) +
  geom_violindot(fill_dots = "black") +
  #geom_jitter2(width = 0.1, alpha = .7) +
  stat_summary(fun = "mean", geom = "point", shape = 18, size = 7, color = "blue") +
  stat_summary(aes(group = 1), geom = "line") +
  scale_x_discrete(labels = c("0" = "Kontrollgruppe", 
                              "1" = "Experimentalgruppe")) +
  labs(x = "",
       y = "fluide Intelligenz")

```

::::

Sowohl das R-Paket `ggstatsplot` als auch das R-Paket `ggpubr` bieten ansprechende Datenvisualisierung.
Beide bauen auf dem R-Paket `ggplot` auf, sind also eigentlich nur "Abkürzungen".
...
Die Punkt- und Intervallschätzer (95%-ETI) für Achsenabschnitt und Regressionsgewicht von `g` sind in @fig-m1-params visualisiert.
...
Das [Farbschema nach Okabe und Ito](https://jfly.uni-koeln.de/color/) ist gut geeignet, um nominal skalierte Farben zu kodieren (s. Details [hier](https://sebastiansauer.github.io/data_se/2023/06/30/farbpaletten/)).
...
In [diesem Blog-Post](https://sebastiansauer.github.io/data_se/2022/06/04/vorher-nachher-messung-und-vergleich-zwischen-gruppen/) findet eine kleine Fallstudie zur Analyse von "Vorher-Nachher-Daten".
...
Im Mediationseffekt (`me`) ist die Null enthalten;
...
- `b`: Pfad b
- `cp`: Pfad c' ("c prime")
- `me`: Mediationseffekt (indirekter Effekt)
...
Mit den "klickbaren" Programmen [Jamovi](https://blog.jamovi.org/2017/09/25/medmod.html) oder [JASP](https://jasp-stats.org/2020/03/12/mediation-and-moderation-analysis-in-jasp/) 
kann man ebenfalls - und zwar sehr einfach - eine Mediationsanalyse durchführen. 
Einfach mal ausprobieren!



## Moderierte Mediation {#sec-moderierte-mediation}

Bei einer *einfachen Mediation* wirkt ein Prädiktor $X$ über einen Mediator $M$
auf ein Ergebnis $Y$. 
Der indirekte Effekt ist das Produkt $a \cdot b$ der
beiden beteiligten Pfade. 
Eine *moderierte* Mediation liegt vor, wenn dieser
indirekte Effekt selbst von einer weiteren Variable, 
dem Moderator $W$, abhängt.
Anders gesagt: Der Mechanismus, über den $X$ auf $Y$ wirkt, ist nicht für alle
gleich stark.

Die zentrale Frage ist immer: *Welcher Pfad wird moderiert?* Diese Entscheidung
ist theoretischer, nicht technischer Natur. Drei Leitfragen helfen:
...
Die ersten beiden Fälle sind moderierte Mediation im engeren Sinn. 
Der dritte
Fall moderiert den direkten Pfad und ist damit eine moderierte *direkte*
Wirkung — dazu unten mehr.

### Konzeptuelles und statistisches Diagramm

In der konzeptuellen Notation nach @hayes2022 zeigt der Moderator mit einem
Pfeil auf einen *Pfad* (nicht auf einen Knoten). 
Diese Darstellung macht die
Hypothese unmittelbar lesbar, 
lässt sich aber nicht in jedem
Diagramm-Werkzeug umsetzen.

```{r}
#| label: fig-modmed-konzeptuell
#| echo: false
#| message: false
#| fig-cap: "Konzeptuelles Diagramm: Der Moderator $W$ zielt auf den a-Pfad (Modell 7)."
#| fig-width: 6
#| fig-height: 4
library(ggplot2)
...
# Pfeil-Funktion für gerade Pfade zwischen Knoten (mit Randabstand)
pfeil <- function(von, nach, daten, kürzen = 0.18) {
  p1 <- daten[daten$name == von, c("x", "y")]
  p2 <- daten[daten$name == nach, c("x", "y")]
  d  <- c(p2$x - p1$x, p2$y - p1$y)
  len <- sqrt(sum(d^2)); u <- d / len
  data.frame(
    x = p1$x + u[1] * kürzen, y = p1$y + u[2] * kürzen,
    xend = p2$x - u[1] * kürzen, yend = p2$y - u[2] * kürzen
  )
}

pfade <- rbind(pfeil("X", "M", knoten),
               pfeil("M", "Y", knoten),
               pfeil("X", "Y", knoten))

ggplot() +
  # Pfade X->M, M->Y, X->Y
  geom_segment(data = pfade,
               aes(x = x, y = y, xend = xend, yend = yend),
               arrow = arrow(length = unit(0.22, "cm"), type = "closed"),
               linewidth = 0.6) +
  # Moderatorpfeil: W zielt auf die Mitte des a-Pfads (X->M)
  geom_segment(aes(x = 1, y = -1.1, xend = 1, yend = 0.6),
               arrow = arrow(length = unit(0.22, "cm"), type = "closed"),
               linewidth = 0.6, colour = "#BA7517") +
  # Pfadbeschriftungen
  annotate("text", x = 0.9, y = 0.85, label = "a", fontface = "italic") +
  annotate("text", x = 3.1, y = 0.85, label = "b", fontface = "italic") +
  annotate("text", x = 2,   y = -0.12, label = "c'", fontface = "italic") +
  # Knoten als Boxen mit Beschriftung
  geom_label(data = knoten, aes(x = x, y = y, label = name),
             size = 6, label.size = 0.4, label.r = unit(0.15, "cm"),
...
Im *statistischen* Diagramm wird die Moderation als Interaktionsterm $X \cdot W$
aufgelöst, der als eigener Prädiktor auf $M$ wirkt. 
Genau das schätzt das
Regressionsmodell `lm(M ~ X * W)`. 
Der Koeffizient dieses Interaktionsterms
heißt $a_3$.

### Der Index der moderierten Mediation

@hayes2015 führt den *Index of moderated mediation* ein. 
Wird der a-Pfad
moderiert, ist der bedingte indirekte Effekt
...
Ist der Index von null verschieden (das Bootstrap-Konfidenzintervall schließt
die Null aus), liegt moderierte Mediation vor. 
Der Test ist also ein Konfidenzintervall auf das Produkt $a_3 \cdot b$.



### Beispiel mit simulierten Daten

Wir erzeugen einen Datensatz, in dem der a-Pfad bewusst moderiert ist
(der Term `0.5 * X * W`):
...
X <- rnorm(n)
W <- rnorm(n)
M <- 0.4 * X + 0.3 * W + 0.5 * X * W + rnorm(n)
Y <- 0.6 * M + 0.2 * X + rnorm(n)
d <- data.frame(X, W, M, Y)
```

#### Schätzung mit `lavaan`

Der Vorteil: Das Pfadmodell steht explizit im Code, 
nichts ist
Blackbox. Der Interaktionsterm wird vorab gebildet:
...
fit <- sem(modell, data = d, se = "bootstrap", bootstrap = 1000)
```

```{r}
#| label: lavaan-ergebnis
parameterEstimates(fit, ci = TRUE) |>
  subset(label %in% c("ind_lo", "ind_mean", "ind_hi", "index"),
         select = c(label, est, ci.lower, ci.upper))
```

Der Parameter `index` ist der Kern: 
Schließt sein Konfidenzintervall die Null
aus, ist die Mediation moderiert. 
Die drei `ind_*`-Werte zeigen, wie stark der
indirekte Effekt bei niedrigem, mittlerem und hohem $W$ ausfällt.

#### Bedingte indirekte Effekte über den Wertebereich von $W$

Statt nur dreier Stützstellen lässt sich der bedingte indirekte Effekt
$(a_1 + a_3 W)\, b$ als stetige Funktion von $W$ darstellen. Das macht
unmittelbar sichtbar, *wo* der Mediationsmechanismus stark ist und wo er
verschwindet:

```{r}
#| label: fig-bedingt-indirekt
#| message: false
#| fig-cap: "Bedingter indirekter Effekt von $X$ auf $Y$ über den Wertebereich des Moderators $W$. Die Steigung der Geraden entspricht dem Index der moderierten Mediation ($a_3 \\cdot b$)."
#| fig-width: 6
#| fig-height: 4
est <- coef(fit)
...
Ist die Gerade waagerecht ($a_3 = 0$), hängt der indirekte Effekt nicht von $W$
ab — keine moderierte Mediation. Je steiler sie verläuft, desto stärker die
Moderation. Genau diese Steigung ist der Index aus @hayes2015.

#### Schätzung mit dem Paket `mediation`

Klassischer Weg über zwei getrennte Modelle:

```{r}
#| label: mediation-paket
#| message: false
#| eval: false
library(mediation)
...
### Sonderfall: Moderation des c′-Pfads

Wird nicht der a- oder b-Pfad, sondern der direkte Pfad c′ moderiert, 
ändert sich der *indirekte* Effekt nicht über $W$ — nur der direkte
Resteffekt von $X$ auf $Y$. 
Das ist inhaltlich eine andere Aussage: 
Der Mediationsmechanismus ist überall gleich stark, 
aber der direkte Effekt hängt
von $W$ ab.

Wichtig: Der *Index of moderated mediation* nach Hayes ist hier **nicht definiert**, weil
der indirekte Effekt konstant bleibt. 
Getestet wird stattdessen der
Interaktionskoeffizient $X \cdot W$ in der $Y$-Gleichung.
...
Der Begriff lautet dann korrekt *moderierte direkte Wirkung*, nicht moderierte
Mediation.
...
Wieso ist der indirekte Effekt definiert als 
...
Die Frage zielt auf den Kern: *Warum* multipliziert man die beiden Pfade überhaupt, und warum genau diese Teile? Ich baue es von Grund auf.


Der indirekte Effekt beantwortet: Wenn ich $X$ um eine Einheit erhöhe, wie stark ändert sich $Y$ — aber *nur über den Umweg durch $M$*, nicht direkt?

Das ist eine zweigliedrige Kette: $X \to M \to Y$. Die Logik dahinter ist eine einfache Verkettung.

#### Die Kettenlogik

Stell dir konkrete Zahlen vor. Angenommen:

- Erhöhe ich $X$ um 1, steigt $M$ um 2 Einheiten.
- Erhöht sich $M$ um 1, steigt $Y$ um 3 Einheiten.

Frage: Erhöhe ich $X$ um 1 — wie viel mehr $Y$ kommt über $M$ heraus?

$X$ schiebt $M$ um 2 hoch. Jede dieser 2 Einheiten $M$ schiebt $Y$ um je 3 hoch. Also $2 \times 3 = 6$. Der Umweg-Effekt ist das **Produkt** der beiden Glieder, nicht die Summe — weil das zweite Glied auf dem Output des ersten aufsetzt. Es ist wie ein Wechselkurs über zwei Stationen: erst Euro→Dollar, dann Dollar→Yen. Den Gesamtkurs bekommst du durch Multiplizieren.

In der Mediation sind diese beiden Glieder:

- Effekt von $X$ auf $M$ = der **a-Pfad**
- Effekt von $M$ auf $Y$ = der **b-Pfad**

Also allgemein:

$$\text{indirekter Effekt} = a \times b$$

#### Wo das $(a_1 + a_3 W)$ herkommt

Bis hierhin galt: $a$ ist eine feste Zahl. Bei der moderierten Mediation ist genau das nicht mehr so. Das Modell für $M$ lautet:

$$M = a_1 X + a_2 W + a_3 (X \cdot W) + \dots$$

Frage wie oben: Wenn $X$ um 1 steigt, wie stark steigt $M$? Sammle alle Terme, in denen $X$ steckt:

$$a_1 X + a_3 (X \cdot W) = (a_1 + a_3 W)\, X$$

Der Faktor vor $X$ ist $a_1 + a_3 W$ — und *das* ist der a-Pfad. Er ist keine feste Zahl mehr, sondern hängt von $W$ ab. Bei $W = 0$ ist er $a_1$; bei $W = 2$ ist er $a_1 + 2a_3$; usw. Das ist die ganze Idee der Moderation: $W$ verstellt die Stärke, mit der $X$ auf $M$ wirkt.

#### Zusammensetzen

Jetzt setze den variablen a-Pfad in die Ketten-Formel $a \times b$ ein:

$$\text{indirekter Effekt}(W) = \underbrace{(a_1 + a_3 W)}_{a\text{-Pfad, hängt von }W\text{ ab}} \times \underbrace{b}_{b\text{-Pfad, fest}}$$

Das $b$ bleibt eine feste Zahl, weil in diesem Modell (Modell 7) nur der a-Pfad moderiert ist — die Y-Gleichung enthält keinen Interaktionsterm mit $M$. Würde stattdessen der b-Pfad moderiert (Modell 14), stünde die Klammer beim $b$ statt beim $a$.

Der indirekte Effekt ist also deshalb $(a_1 + a_3 W)\, b$, weil er das Produkt zweier verketteter Glieder ist — und das erste Glied durch die Moderation selbst eine Funktion von $W$ geworden ist.
...
- [research-question](https://datenwerk.netlify.app/#category=research-question) 
- [researchdesign](https://datenwerk.netlify.app/#category=researchdesign)
