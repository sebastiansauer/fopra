...
Berechnen Sie also *nicht* für jede UV ein eigenes Modell, sondern packen Sie alle UV in ein einziges Regressionsmodell -- sofern Sie der Meinung sind, alle UV sind direkte Ursachen der AV.
Variablen mit anderen kausalen Funktionen, wie Konfundierungs- und Mediatorvariablen, sollten nicht in Ihr Modell aufgenommen werden, mit dem Sie den Effekt der UV auf die AV untersuchen.
Es ist auch grundsätzlich sinnvoll, zusätzlich erhobenen Variablen, wie soziodemografische Variablen, in Ihr Modell aufzunehmen.
...
%%| fig-cap: "Ablauf der Studie zur Behaltensleistung y2 in Abhängigkeit der Visualisierungsart V (Within-Variable)"
flowchart LR
  V.1 --> y1 --> V.2 --> y2
```
