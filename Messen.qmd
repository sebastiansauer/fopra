...
library(plotly)  # Animation
```


```{r}
data("airquality")  # Animination
data("gapminder", package = "gapminder")  # Animation
```
...
Das Instrument besteht aus zwei (korrelierten) Skalen: Muskulatur-bezogene Kognition und Muskulatur-bezogenes Verhalten; der Volltext und weitere Informationen findet sich [hier](https://zis.gesis.org/skala/Waldorf-Cordes-Vocks-McCreary-Deutschsprachige-Drive-for-Muscularity-Scale-(DMS)).
...
### Fallbeispiel Mensch-Maschine-Interaktion
...
Um die Items für ein bestimmtes System anzupassen, sind (geringfügige) Änderungen sinnvoll.
...
Die Skala stammt wiederum aus der Studie von @natour_adoption_2011 [Volltext hier](https://www.researchgate.net/profile/Sameh-Al-Natour/publication/220580434_The_Adoption_of_Online_Shopping_Assistants_Perceived_Similarity_as_an_Antecedent_to_Evaluative_Beliefs/links/0deec521a214ec8489000000/The-Adoption-of-Online-Shopping-Assistants-Perceived-Similarity-as-an-Antecedent-to-Evaluative-Beliefs.pdf?_sg%5B0%5D=started_experiment_milestone&origin=journalDetail&_rtd=e30%3D).
...
#### Neue Messung selber entwickeln
...
*Latente* Variablen, die also nicht direkt beobachtbar sind, sind schwer zu messen. Psychologische Variablen gehören in der Regel dazu. Daher sollten Sie solche Variablen nicht mit eigenen, selbst entwickelten Instrumenten erheben. Das Problem ist, dass es unklar ist, ob Ihr "Messgerät" funktioniert.
Viel besser ist in diesem Fall, auf bestehende Messgeräte zurückzugreifen.

Beispiele für Variablen, die Sie lieber mit existierenden Messinstrumenten messen:
...
Werte ab .7 werden mitunter als akzeptabel und ab .8 als gut eingeschätzt.$\square$
:::
...
Eine in der experimentellen Psychologie häufig eingesetzte Sammlung an Bildern ist der *International affective picture system (IAPS)*  [@Lang1997].
...
`gganimate` erstellt eine große Zahl von `ggplot`-Diagrammen, von denen jeweils eines als Bild im "Film" einer Animation gezeigt wird. Man kann die Bildern dann als GIF-Bild speichern.
...
Und hier die animierte Variante:
...
[Plotly](https://plotly.com/r/) ist eine Browser-basierte Methode, die das Bild dynamisch im Browser erzeugt (und nur dort).
...
das die Veränderung im Zeitverlauf der Lebenserwartung in Abhängigkeit des Bruttosozialprodukts für viele Länder zeigt.
...
Das ggplot-Objekt können wir jetzt einfach in ein Plotly-Objekt übersetzen lassen:
...
was auch nicht so schwierig ist.
...
So bietet `dygraphs` interaktive - aber nicht animierte - Diagramme.
...
Neu dabei ist [Observable](https://observablehq.com/@slopp/observable-for-r-users), womit auch browserbasierte Diagramme erstellt werden können. Eigentlich ist es ein JavaScript-Tool, aber es gibt eine R-Andbindung, [RObservalbe](https://cran.r-project.org/web/packages/robservable/vignettes/introduction.html):
